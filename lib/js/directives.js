;(function () {
  ...
  angular.module('pdb.topology.viewer', ['d3Core', 'pdb.common.services', 'template/topologyViewer/topologyViewer.html'])
	.directive('pdbTopologyViewer', ['d3', '$filter', 'commonServices', '$document', function(d3, $filter, commonServices, $document){
      	...
      	link: function (scope, element, attrs) {
			...
			var TopologyComponent = (function () {
				...
				TopologyComponent.prototype.getTopologyApiData = function () {
					...
					commonServices.combinedDataGrabber(ratioPromiseList, scope.entryId, ['observedResidueRatio'], true).then(function(ratioResult) {
						...
						if(typeof ratioResult[scope.entryId].observedResidueRatio[scope.entityId] !== 'undefined'){
							...
							var pdbPromiseList = commonServices.createPromise([scope.entryId], pdbApiNameList, scope.bestChainId);
							commonServices.combinedDataGrabber(pdbPromiseList, scope.entryId, pdbApiNameList, true).then(function(result) {
								...
								if(typeof result[scope.entryId]['topology'] == 'undefined' || result[scope.entryId]['topology'] == null){
									topologyDataError = true;
								}else if(typeof result[scope.entryId]['topology'][scope.entityId] == 'undefined' || 
									typeof result[scope.entryId]['topology'][scope.entityId][scope.bestChainId] == 'undefined'){
									topologyDataError = true;
								}
								...
								_this.getPDBSequenceArray(result[scope.entryId]['entities']);
								_this.drawTopologyStructures();
								_this.createDomainDropdown();
							}, function() {
								_this.apiResult = { status: 'error', data: 'Combined api call failed' }
								scope.overlayText = 'Error: combined api call failed!';
							});
				...
				TopologyComponent.prototype.getDomainRange = function () {
					...
					angular.forEach(this.apiResult.data[scope.entryId].topology[scope.entityId][scope.bestChainId], function(secStrArr, secStrType) {
				...
				TopologyComponent.prototype.drawCoilsSubpaths = function (startResidueNumber, stopResidueNumber, index) {
					...
					var termsData = this.apiResult.data[scope.entryId].topology[scope.entityId][scope.bestChainId].terms;
					var totalCoilsInStr = this.apiResult.data[scope.entryId].topology[scope.entityId][scope.bestChainId].coils.length
					if(index === 0){
			...
			TopologyComponent.prototype.showSelectionArea = function (residueNumber, persistPreviousSelection, colorArr, eventType) {
				...
				//Display clear icon
				directiveEle.select('.topologyRefreshIcon').style('display','block');
			};
			...
			TopologyComponent.prototype.getAnnotationFromMappings = function () {
				var mappingsData = this.apiResult.data[scope.entryId].mappings;
			...
			TopologyComponent.prototype.getChainStartAndEnd = function () {
				...
				var chainsData = this.apiResult.data[scope.entryId].polymerCoveragePerChain.molecules[0].chains;
			...
			TopologyComponent.prototype.getAnnotationFromOutliers = function () {
				...
				var outlierData = this.apiResult.data[scope.entryId].outliers;
        ...
        var topologyComponent = new TopologyComponent();
		
		//Method for domain dropdown change 
        scope.highlightDomain = function(){
          topologyComponent.highlightDomain();
		}
		...
		if(scope.subscribeEvents == 'true'){
			...
			$document.on('PDB.seqViewer.click', function(e){
				if(typeof e.eventData !== 'undefined'){
					...
					if(e.eventData.entryId != scope.entryId || e.eventData.entityId != scope.entityId || e.eventData.viewerType == 'unipdbViewer') return;
					
					if(typeof e.eventData.elementData !== 'undefined' && elementTypeArrForSingle.indexOf(e.eventData.elementData[0].elementType) > -1){
						//Abort if chain id is different
						if(e.eventData.elementData[0].pathData.chain_id != scope.bestChainId)return;
						topologyComponent.showSelectionArea(e.eventData.residueNumber, false, undefined, 'click')
					}else if(typeof e.eventData.elementData !== 'undefined' && elementTypeArrForRange.indexOf(e.eventData.elementData[0].elementType) > -1){
						//Residue range
						var startResidue = e.eventData.elementData[0].pathData.start.residue_number;
						var endResidue = e.eventData.elementData[0].pathData.end.residue_number;
						...
						for(var rangeIndex = startResidue; rangeIndex <= endResidue; rangeIndex++){
							...
							topologyComponent.showSelectionArea(rangeIndex, persistPreviousSelection, e.eventData.elementData[0].color, 'click');
						}
			...
			$document.on('PDB.seqViewer.mouseover', function(e){
				if(typeof e.eventData !== 'undefined'){
					...
					if(typeof e.eventData.elementData !== 'undefined' && elementTypeArrForSingle.indexOf(e.eventData.elementData[0].elementType) > -1){
						//Abort if chain id is different
						if(e.eventData.elementData[0].pathData.chain_id != scope.bestChainId)return;
						topologyComponent.showSelectionArea(e.eventData.residueNumber, false, undefined, 'mouseover')
					}else if(typeof e.eventData.elementData !== 'undefined' && elementTypeArrForRange.indexOf(e.eventData.elementData[0].elementType) > -1){
						//Residue range
						var startResidue = e.eventData.elementData[0].pathData.start.residue_number;
						var endResidue = e.eventData.elementData[0].pathData.end.residue_number;
						...
						for(var rangeIndex = startResidue; rangeIndex <= endResidue; rangeIndex++){
							var persistPreviousSelection = true;
							if(rangeIndex == startResidue)persistPreviousSelection = false;
							topologyComponent.showSelectionArea(rangeIndex, persistPreviousSelection, e.eventData.elementData[0].color, 'mouseover');
						}
